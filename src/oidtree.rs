...
impl OidTree {
    ...
    pub fn oid_by_name_under(
         ...
         */
        let terminus = self.walk_down_under(root, &t).map_err(|e| {
            anyhow!("mapping OID {name:?} under {parent:?}: {e}")
        })?;

        Ok(self.oid_for_node(terminus))
    }

    pub fn oid_by_name(&self, name: &str) -> Result<ObjectIdentifier> {
        let t = split_name(name)?;
    ...
    pub fn oid_name(&self, oid: &ObjectIdentifier) -> Result<OidName> {
